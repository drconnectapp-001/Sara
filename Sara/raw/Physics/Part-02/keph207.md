...
communications essentially depend on transmission of signals through waves. Speech means production of sound
waves in air and hearing amounts to their detection. Often,
communication involves different kinds of waves. For example, sound waves may be first converted into an electric current signal which in turn may generate an electromagnetic
...
satellite. Detection of the original signal will usually involve these steps in reverse order.
...
δ*p*, in that region. Pressure is force per unit area,
...
spring sideways.
(b)
Waves produced in a cylinder
containing a liquid by moving its piston
back and forth.
(c)
Waves produced by a motorboat sailing
in water.
(d)
Ultrasonic waves in air produced by a
...
fixed instant,  say *t* = *t*0. Then, the argument of
...
wave. Similarly, take a fixed location, say *x* = *x*0.
Then, the argument of the sine function in
Equation (14.2) is constant -w*t*. The
...
*kx* – w*t* + f  constant. Thus, Eq. (14.2) represents
a sinusoidal (harmonic) wave travelling along
...
The quantity (*kx* – w*t* + f) appearing as the
...
and the initial time, it is possible to have f = 0.
Thus there is no loss of generality in dropping
f, i.e., in taking Eq. (14.2) with f = 0.
...
*
| Here again, ‘radian’ could be dropped and the units could be written merely as m | –1 | . Thus, k represents 2 | π |
| --- | --- | --- | --- |
...
u **Example 14.2**  A wave travelling along a
...
we find
(a)
the amplitude of the wave is 0.005 m = 5 mm.
(b)
the angular wave number *k* and angular
frequency ω are
...
(c)
Now, we relate *T*to ω by the relation
...
u **Example 14.3** A steel wire 0.72 m long
...
*V*, *k**B* is the Boltzmann constant and *T*the
...
ρ*o* = (mass of one mole of air)/ (volume of one
mole of air at STP)
...
D(*PV*g )  = 0
or
*P* g*V* g*–1* D*V* + *V* g D*P*= 0

where g is the ratio of two specific heats,
Cp/Cv.
...
g = 7/5. Now using Eq. (14.24) to estimate the speed
...
the medium, therefore, suffer displacements due
...
mathematically, let *y*1 (*x*,*t*) and *y*2 (*x*,*t*) be the
...
the same w (angular frequency) and *k* (wave
number), and, therefore, the same wavelength
l. Their wave speed will be identical. Let us
...
*y*1(*x, t)*= *a*sin (*kx –*w*t*)
(14.27)

and *y*2(*x, t)*=  *a*sin (*kx –*w*t +*f )
(14.28)

The net displacement is then, by the principle
of superposition, given by

*y*  (*x, t*) = *a*sin (*kx –*w*t*)*+ a*sin (*kx –*w*t +*f)
(14.29)
...

(14.30)
where we have used the familiar trigonometric
...
f between the constituent two waves:
*A*(f)*=* 2*a* cos ½f
(14.32)
For f = 0, when the waves are in phase,
...
*depends on the phase difference*f*, which*
*is zero  for (a) and*p*for (b)*
...
*y*1(*x, t*) = *a*sin (*kx –*ω*t*)

*y*2(*x, t*) = *a*sin (*kx +*ω*t*)
...
kx and w*t* appear separately, not in the
combination *kx* - w*t*. The amplitude of this wave
is 2*a*sin *kx*. Thus, in this wave pattern, the
amplitude varies from point-to-point, but each
element of the string oscillates with the same
angular frequency w or time period. There is no
...
u **Example 14.5**  A pipe, 30.0 cm long, is open
...
resonate at *v*2, i.e. the **second harmonic***.*
...
angular frequency w1 and w2 and fix the location
to be x = 0 for convenience. Eq. (14.2) with a
suitable choice of phase (f = p/2 for each) and,
assuming equal amplitudes, gives
*s*1 = *a* cos w1*t*and  *s*2 = *a* cos w2*t*(14.45)
Here we have replaced the symbol y by *s*,
since we are referring to longitudinal not
transverse displacement. Let w1 be the (slightly)
greater of the two frequencies. The resultant
displacement is, by the principle of
superposition,
*s*=  *s*1*+ s*2 = *a*(cos w1 *t +*cos w2 *t*)
Using the familiar trigonometric identity for
cos A + cosB, we get
...
*s =* [2*a*cos w*b**t* ] cos w*a**t*
(14.47)
If |w1 *–* w2| <<w1, w2, w*a**>>*w*b*, th
where
...
Now if we assume |w1 *–* w2| <<w1, which means
w*a**>>*w*b*, we can interpret Eq. (14.47) as follows.
The resultant wave is oscillating with the average
angular frequency wa; however its amplitude is
**not** constant in time, unlike a pure harmonic
wave. The amplitude is the largest when the
term *cos*w*b**t* takes its limit +1 or –1. In other
words, the intensity of the resultant wave waxes
and wanes with a frequency which is 2wb = w1 –
...
ω2. Since  ω*=*2πν, the beat frequency ν*beat*, is
given by
ν*beat* = ν1 – ν2
(14.48)
...
of B (ν*B*) were greater than that of A (ν*A*), further
increase in ν*B* should have resulted in an
increase in the beat frequency. But the beat
frequency is found to decrease. This shows that
ν*B* < ν*A*. Since ν*A* – ν*B* = 5 Hz, and ν*A* = 427 Hz, we
get ν*B* = 422 Hz.
⊳

**Musical Pillars**
Temples often have some pillars portraying
human figures playing musical instruments, but seldom do these pillars
...
| --- | --- | --- |
| oscillation of such a system. The lowest frequency given by | v/ | 4 | L | is the fundamental |

mode or the first harmonic.

16.
A string of length *L* fixed at both ends or an air column closed at one end and open
...
17.
| Beats | arise when two waves having slightly different frequencies, | ν | 1 | and | ν | 2 | and |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
**14.9**
| For the wave described in Exercise 14.8, plot the displacement ( | y | ) versus ( | t | ) graphs |
| --- | --- | --- | --- | --- |
...
**14.12**
(i) For the wave on a string described in Exercise 15.11, do all the points on the
...
(d)
*y*= cos *x*sin *t* + cos 2*x*sin 2*t*

**14.14**
A wire stretched between two rigid supports vibrates in its fundamental mode with
...
(b)
bats can ascertain distances, directions, nature, and sizes of the obstacles
without any “eyes”,
