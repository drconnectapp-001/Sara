...
*Fig. 5.1*
| (a) | The scalar product of two vectors | A | and | B | is a scalar : | A | . | B | = A B | cos | θ | . | (b) | B | cos | θ | is the projection |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
*Fig. 5.2*
*An object undergoes a displacement*d
...
where *K**i*  and *K**f*  are respectively the initial and
...
an angle of 180o  (p rad) with each other.
...
No work is done if :
(i)
the displacement is zero as seen in the
...
perpendicular. This is so since, for q = p/2
rad (= 90o), cos (p/2) = 0.  For the block
...
radially inwards and q  = p/2.
Work can be both positive and negative.  If q  is
between 0o and 90o, cos q  in Eq. (5.4) is positive.
If  q  is  between 90o and 180o,   cos q  is negative.
In many examples the frictional force opposes
displacement and q  = 180o. Then the work done
...
is *mv**2*/2 = 1000 J.  It has a final kinetic energy
of 0.1×1000 = 100 J.  If *v**f*  is the emergent speed
of the bullet,
...
*Fig. 5.4*
*Plot of the force F applied by the woman and*
...
where,  *K**i*  and *K**f*  are the initial and final kinetic
...
*v**i* = 2 m s–1 enters a rough patch ranging
...
energy and speed *v**f*  of the block as it
crosses this patch ?
...
compared to the earth’s radius *R**E*(*h <<R**E*) so that
...
is
*gh*
irrespective of the angle of inclination.
...
Over the whole path,*x**i* to *x**f**,*this means that
*K**i* + *V*(*x**i* ) = *K**f*  + *V*(*x**f*)
(5.11)
...
The total mechanical energies *E*0, *E**h*, and *E**H*
...
expression for (i) *v**o*; (ii)  the speeds at points
B and C; (iii) the ratio  of  the kinetic energies
(*K**B**/K**C*) at B and C. Comment on the nature
...
where *T**A* is the tension  in the string at A. At the
...
where *v**C* is the speed at C. From Eqs. (5.13)
and (5.14)
...
spring, the spring force *F**s*  is proportional to
...
Fig. 5.7(b). If the extension is *x**m*, the work done by
the spring force is
...
compressed with a displacement *x**c* (< 0).  The
...
external force *F* does work + *kx**c*

*2* / 2. If the block
is moved from an initial displacement *x**i*  to a
final displacement *x**f* , the work done by the
spring force *W**s* is
...
is pulled from *x**i*  and allowed to return to *x**i* ;
...
only as first stated by Hooke, (*F**s**=*− *kx*); (ii)
...
You may easily verify that − d*V/*d*x =* −*k x*, the
spring force.  If the block of mass *m* in Fig. 5.7 is
extended to *x**m* and released from rest, then its
total mechanical energy at any arbitrary point *x*,
where *x* lies between *– x**m* and *+ x**m*, will be given by
...
Now  µ*mg*  = 0.5 × 103 × 10 = 5 × 103 N (taking
...
*x**m* is positive. Putting in numerical values we
obtain
*x**m*  *=*1.35 m
...
force  *F**nc* , the conservation of mechanical energy
...
*F = m g* +*F**f* *=*(1800 × 10) + 4000 = 22000 N
...
Consider two masses *m**1* and *m**2*.  The particle
*m**1* is moving with speed *v**1i* , the subscript ‘*i*’
implying initial. We can cosider *m**2*  to be at rest.
No loss of generality is involved in making such
a selection. In this situation the  mass *m**1*
collides  with  the stationary  mass *m**2*  and  this
is depicted in  Fig. 5.10.

*Fig. 5.10**Collision of mass**m**1**, with a stationary mass**m**2**.*
...
unlike the conservative force, *W**nc*depends on
...
*d*r is d*W*= F.*d*r.  The instantaneous power can
...
where **F**12 is  the force exerted on the first particle
by the second particle. **F**21 is likewise the force
exerted on the second particle by the first particle.
Now from Newton’s third law, **F**12 = − **F**21.  This
implies
...
*m*1*v*1*i**=*(*m*1*+m*2)*v**f*  (momentum conservation)
...
Thus, the ‘unknowns’ {*v*1*f*, *v**2f*} are obtained in
terms of the ‘knowns’ {*m*1, *m*2, *v*1*i*}. Special cases
...
moderating nuclei *K**2f*/*K**1i*  is
...
from Eq. (5.27).
For deuterium *m**2**=*2*m*1 and we obtain
*f*1*=*1/9  while *f**2**=*8/9.  Almost 90% of the
neutron’s energy is transferred to deuterium. For
carbon *f*1*=*71.6% and *f*2 *=* 28.4%.  In practice,
...
mass *m*1 with the stationary mass *m*2. Linear
...
velocity directions of *m*1 and *m*2 and choose it to
...
*m*1*v*1*i**= m*1*v*1*f* cos θ 1*+ m*2*v*2*f* cos θ 2     (5.28)

0  *= m*1*v*1*f*  sin θ1 −*m*2*v*2*f* sin θ2        (5.29)

One knows {*m*1*, m*2*, v*1*i*} in most situations.  There
are thus four unknowns {*v*1*f**, v*2*f**,*θ1 *and*θ2}, and
...
{*m*1, *m*2, *v**1i* , θ1} we can determine {*v**1f**, v**2f**,*θ*2*}
from Eqs. (5.28)-(5.30).

**Example 5.12** Consider the collision
depicted in Fig. 5.10 to be between two
billiard balls with equal masses *m**1**= m**2*.
...
Since the collision is elastic and *m**1**= m**2* it follows
...
| --- | --- | --- | --- | --- | --- | --- |
| quantity given by : | A | . | B | = | AB | cos | q | , | where | q | is the angle between | A | and | B | .  It can be |
| positive, negative or zero depending upon the value of | q | . The scalar product of two |
...
| energy is zero is a matter of choice. For the gravitational  potential energy | m | g | h | , |
| --- | --- | --- | --- | --- |

the zero of the potential energy is chosen to be the ground. For the spring
potential energy *kx**2**/*2 , the zero of the potential energy is the equilibrium position
...
velocity,
(e)
work done by the resistive force of
...
(c)
work done by the net force on the
body in 10 s,
...
*kx**2**/*2, where *k*  is the force constant
...
energy.
(c)
The rate of change of total momentum of a many-particle system is proportional
...
| 5.15 | A pump on the ground floor of a building can pump up water to fill a tank of volume 30 m | 3 |
| --- | --- | --- |
...
| 5.20 | A body of mass 0.5 kg travels in a straight line with velocity | v | = | a | x | 3/2 | where | a = | 5 | m | –1/2 | s | –1 | . |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| converts 25% of the wind’s energy into electrical energy, and that | A | = 30 m | 2 | , | v | = 36 |
| --- | --- | --- | --- | --- | --- | --- |
