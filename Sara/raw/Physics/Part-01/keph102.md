...
D*t* becomes infinitesimally small. In other words,
...
of taking limit as  D*t*g0 of the quantity on its
...
Let us take D*t* = 2 s centred at *t*= 4 s. Then,
by the definition of the average velocity, the
slope of line P1P2  ( Fig. 2.1) gives the value of
average velocity over the interval   3 s to 5 s.

**Fig. 2.1**
*Determining velocity from position-time*
*graph.  Velocity at t =*4 s*is the slope of the*
*tangent to the graph at that instant*.

Now, we decrease the value of D*t* from 2 s to 1
s.  Then line P1P2 becomes Q1Q2   and its slope
gives the value of the average velocity over
the interval 3.5 s to 4.5 s. In the limit D*t* ® 0,
the line P1P2 becomes tangent to the positiontime curve at the point P and the velocity at *t*
...
in Fig. 2.1, *x*= 0.08 *t*3.  Table 2.1 gives the
value of D*x*/D*t*calculated for D*t* equal to 2.0 s,
...
corresponding values of *x*, i.e. *x*(*t*1) = 0.08 *t*1

and *x*(*t*2) = 0.08 *t*2
3. The sixth column lists the
difference ∆*x* =*x*(*t*2) –*x*(*t*1) and the last
...
*t*= 4.0 s, i.e. the value of  *dx*

*dt*  at *t*= 4.0 s. In this
...
*dt*  at different instants as done in

the following example.

**Example 2.1***The position of an object*
*moving along x-axis is given by   x = a + bt*2
...
+ 24.0 m s–1 and a velocity of – 24.0 m s–1 —
both have an associated speed of 24.0 m s-1.  It
...
or simply velocities at time  *t*2 and *t*1 . It is the
...
connecting the points corresponding to (*v*2*, t*2)
and (*v*1*, t*1).
...
*direction at time t**1**.  Between times 0 to*
*t**1**, it moves in  positive x - direction*
*and between t**1**and t**2**it moves in the*
*opposite direction*.
...
for some simple cases. Fig. 2.3 shows     velocitytime graph for motion with constant acceleration
for the following cases :
(a)
An object is moving in a positive direction
with a positive acceleration.
(b)
An object is moving in positive direction
with a negative acceleration.

(c)
An object is moving in negative direction
with a negative acceleration.
(d)
An object is moving in positive direction
till time*t**1**,* and then turns back with the
...
(*x*), time taken (*t*), initial velocity (*v**0*), final
velocity (*v*) and acceleration (*a*). Equation (2.4)
already obtained gives a relation between final
and initial velocities *v* and  *v**0* of an object moving
...
From Eq. (2.4),*t =*(*v – v**0*)*/a*. Substituting this in
Eq. (2.7a), we get
...
connecting five quantities *v*0*,  v, a, t* and *x*.  These
...
we take  the  position coordinate at *t* = 0 as nonzero, say *x*0.  Then Eqs. (2.9a) are modified
(replacing *x* by *x*– *x*0 ) to :
...
calculate the corresponding time taken *t*1 and
*t*2.  Since the velocity  at B is zero, we have :
*v* =*v*o*+  at*
0 = 20  – 10*t*1
Or,
*t*1 = 2 s
...
We have, *y*0 = 45 m, *y*= 0, *v**0* = 0,*a = – g*  = –10 m s–2
...
it hits the ground = *t*1 *+*   *t*2  =  2  s + 3 s = 5 s.
...
*v**0* = 0 and the equations of motion become:

*v* =  0 – *g t*
= –9.8 *t*      m s–1

*y* =  0 – *½*  *g t*2   = –4.9 *t* 2    m
*v*2 = 0 – 2 *g y*     = –19.6 *y*   m2 s–2
...
(–1/
2) *g*τ2 as*y**0*— the position coordinate after
first time interval τ, then third column gives
the positions in the unit of *y**o*. The fourth
...
Therefore, *v**o* = 0, and *a* = –*g* = –9.8 m s–2. The
distance travelled *d* and the reaction time *t**r* are
related by
...
5.
| The area under the velocity-time curve between times | t | 1 | and | t | 2 | is equal to the displacement |
| --- | --- | --- | --- | --- | --- | --- |
...
| x | , time taken | t | , initial velocity | v | 0 | , final velocity | v | and acceleration | a | are related by a set |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
1.
The origin and the positive direction of an axis are a matter of choice. You should first specify
...
5.
In the kinematic equations of motion [Eq. (2.9)], the various quantities are algebraic,
...
6.
The definitions of instantaneous velocity and acceleration (Eqs. (2.1) and (2.3)) are
...
a particle.
**2.13**
Figure 2.11shows the *x-t* plot of onedimensional motion of a particle. Is it correct
